...
export default function TestimonialsCarousel() {
  ...
  return (
    <section id="testimonials" className="relative">
      ...
      {isModalOpen && (
        <Modal isOpen={isModalOpen} onClose={closeModal}>
          <h2 className="text-2xl font-bold mb-4">{selectedTestimonial?.name}</h2>
          <div className="text-1xl font-medium mb-4 font-italic">{selectedTestimonial?.title}</div>
          <p className="text-[#546A7B]">{selectedTestimonial?.content}</p>
        </Modal>
      )}
    </section>
  );
